# 𝘚𝘩𝘰𝘱𝘪𝘧𝘺 𝘝𝘐𝘗 𝘚𝘺𝘴𝘵𝘦𝘮 - (𝟭𝟱𝗪 - 𝟭𝟬𝟬% 𝘈𝘤𝘤𝘶𝘳𝘢𝘤𝘺 - 𝘚𝘵𝘢𝘣𝘭𝘦 𝘊𝘗𝘔)
...
try: HITS_GROUP_ID = int(_hgid)
except: HITS_GROUP_ID = _hgid
...
async def get_msg_lock():
    global _MSG_LOCK
    if _MSG_LOCK is None: _MSG_LOCK = asyncio.Lock()
    return _MSG_LOCK

async def get_edit_lock():
    global _EDIT_LOCK
    if _EDIT_LOCK is None: _EDIT_LOCK = asyncio.Lock()
    return _EDIT_LOCK

async def get_keys_lock():
    global _KEYS_LOCK
    if _KEYS_LOCK is None: _KEYS_LOCK = asyncio.Lock()
    return _KEYS_LOCK
        ...
        if use_gif or specific_gif:
            for _ in range(3): 
                ...
                try:
                    ...
                    if gif_file: return await event.client.send_file(event.chat_id, gif_file, caption=text, buttons=buttons, reply_to=event.message.id, parse_mode="html", attributes=[DocumentAttributeAnimated()])
                    else: return await event.client.send_file(event.chat_id, target_url, caption=text, buttons=buttons, reply_to=event.message.id, parse_mode="html")
                except FloodWaitError as e: await asyncio.sleep(e.seconds + 1)
                except Exception: await asyncio.sleep(0.5)
            return None
        else:
            try: return await event.reply(text, buttons=buttons, parse_mode="html")
    ...
    async with lock:
        if use_gif or specific_gif:
            for _ in range(3): 
                ...
                try:
                    gif_file = await fetch_gif_to_memory(target_url)
                    if gif_file: return await client_instance.send_file(chat, gif_file, caption=text, buttons=buttons, parse_mode="html", attributes=[DocumentAttributeAnimated()])
                    else: return await client_instance.send_file(chat, target_url, caption=text, buttons=buttons, parse_mode="html")
                except FloodWaitError as e: await asyncio.sleep(e.seconds + 1)
                except Exception: await asyncio.sleep(0.5)
            return None
        else:
            try: return await client_instance.send_message(chat, text, buttons=buttons, parse_mode="html")
            except FloodWaitError as e:
                await asyncio.sleep(e.seconds + 1)
...
async def styled_edit(msg, text, buttons=None):
    ...
    async with lock:
        try: return await msg.edit(text, buttons=buttons, parse_mode="html")
        except FloodWaitError as e:
            await asyncio.sleep(e.seconds + 1)
            return None 
        except Exception: return None
...
def parse_proxy_format(proxy):
    ...
    elif re.match(r'^([^:@]+):(\d+)$', proxy): h, p = re.match(r'^([^:@]+):(\d+)$', proxy).groups()
    else: return None
...
async def is_maintenance(event):
    if _MAINTENANCE_MODE and event.sender_id not in ADMIN_ID:
        msg = f"⦗ {CE_11} ⦘ 𝘚𝘺𝘴𝘵𝘦𝘮 𝘔𝘢𝘪𝘯𝘵𝘦𝘯𝘢𝘯𝘤𝘦\n\n├ 𝘛𝘩𝘦 𝘣𝘰𝘵 𝘪𝘴 𝘤𝘶𝘳𝘳𝘦𝘯𝘵𝘭𝘺 𝘶𝘯𝘥𝘦𝘳𝘨𝘰𝘪𝘯𝘨 𝘶𝘱𝘨𝘳𝘢𝘥𝘦𝘴.\n╰ 𝘗𝘭𝘦𝘢𝘴𝘦 𝘵𝘳𝘺 𝘢𝘨𝘢𝘪𝘯 𝘭𝘢𝘵𝘦𝘳."
        if isinstance(event, events.CallbackQuery.Event):
            await event.answer("Maintenance Break!", alert=True)
            await styled_edit(event, msg)
        else: await styled_reply(event, msg, use_gif=True)
        return True
    return False
...
async def check_card_api(card, site, proxy, session, gateway_name):
    try:
        ...
        if status == 'Charged' or 'order completed' in response_lower or 'thank you' in response_lower or 'payment successful' in response_lower: 
...
async def check_card_with_retry(card, sites, proxies, session, gateway_name, max_retries=2):
    ...
    for attempt in range(max_retries):
        ...
        if not active_sites: _SITE_ERRORS_COUNT.clear(); active_sites = sites
...
async def _send_global_hit(status, gateway, message, price, uid, elapsed):
    try:
        if str(HITS_GROUP_ID) in ["0", ""]: return
        try: user = await client_instance.get_entity(uid); user_name = getattr(user, 'first_name', f"User {uid}")
...
async def start(event):
    ...
    try:
        ...
        admin_panel = f"\n\n⦗ {CE_3} ⦘ 𝘈𝘥𝘮𝘪𝘯 𝘗𝘢𝘯𝘦𝘭\n├ /gen [plan] [qty]\n├ /validate [key]\n├ /users\n╰ /maint" if event.sender_id in ADMIN_ID else ""
...
⦗ {CE_9} ⦘ 𝘠𝘰𝘶𝘳 𝘗𝘭𝘢𝘯 ⇾ <code>{plan.title() if plan else 'Bronze'} ({limit} 𝘓𝘪𝘮𝘪𝘵)</code>"""
    ...
    except Exception as e: await event.reply(f"⚠️ 𝘌𝘳𝘳𝘰𝘳: {e}")
...
async def show_plans(event):
    ...
    kb = [[Button.url("⦗ 👑 ⦘ 𝘊𝘰𝘯𝘵𝘢𝘤𝘵 𝘖𝘸𝘯𝘦𝘳", "https://t.me/Dddadddyttt")], [Button.inline("⦗ 🔙 ⦘ 𝘉𝘢𝘤𝘬", b"back_start")]]
...
async def feedback_cmd(event):
    ...
    if not text and not event.is_reply and not getattr(event.message, 'media', None): 
        return await styled_reply(event, f"⚠️ 𝘗𝘭𝘦𝘢𝘴𝘦 𝘱𝘳𝘰𝘷𝘪𝘥𝘦 𝘢 𝘮𝘦𝘴𝘴𝘢𝘨𝘦.", use_gif=True)
    ...
    if admin:
        try:
            if event.is_reply:
                ...
                if text: await client_instance.send_message(admin, f"💬 𝘕𝘰𝘵𝘦: {text}\n📩 𝘍𝘳𝘰𝘮: <code>{uid}</code>", parse_mode="html")
                else: await client_instance.send_message(admin, f"📩 𝘍𝘦𝘦𝘥𝘣𝘢𝘤𝘬 𝘍𝘳𝘰𝘮: <code>{uid}</code>", parse_mode="html")
            else:
                await client_instance.forward_messages(admin, event.message)
                await client_instance.send_message(admin, f"📩 𝘍𝘦𝘦𝘥𝘣𝘢𝘤𝘬 𝘍𝘳𝘰𝘮: <code>{uid}</code>", parse_mode="html")
        except Exception: pass
            
    await styled_reply(event, f"⦗ {CE_4} ⦘ 𝘠𝘰𝘶𝘳 𝘮𝘦𝘴𝘴𝘢𝘨𝘦 𝘩𝘢𝘴 𝘣𝘦𝘦𝘯 𝘥𝘦𝘭𝘪𝘷𝘦𝘳𝘦𝘥 𝘵𝘰 𝘵𝘩𝘦 𝘖𝘸𝘯𝘦𝘳.", use_gif=True)
...
async def check_joined_cb(event):
    ...
    if await is_user_joined(uid):
        ...
        await styled_send(event.chat_id, f"⦗ {CE_2} ⦘ 𝘚𝘩𝘰𝘱𝘪𝘧𝘺 𝘝𝘐𝘗 𝘚𝘺𝘴𝘵𝘦𝘮\n╰ 𝘚𝘦𝘯𝘥 /start 𝘵𝘰 𝘷𝘪𝘦𝘸 𝘵𝘩𝘦 𝘮𝘦𝘯𝘶.", use_gif=True)
...
async def add_proxy_cmd(event):
    ...
    try:
        ...
        if event.is_reply:
            ...
            else: return await styled_reply(event, f"⚠️ 𝘗𝘭𝘦𝘢𝘴𝘦 𝘱𝘳𝘰𝘷𝘪𝘥𝘦 𝘵𝘩𝘦 𝘱𝘳𝘰𝘹𝘪𝘦𝘴 𝘤𝘰𝘳𝘳𝘦𝘤𝘵𝘭𝘺.", use_gif=True)
        
        if not lines: return await styled_reply(event, f"⚠️ 𝘕𝘰 𝘱𝘳𝘰𝘹𝘪𝘦𝘴 𝘧𝘰𝘶𝘯𝘥.", use_gif=True)
        ...
        if cc >= 100: return await styled_reply(event, f"⚠️ 𝘓𝘪𝘮𝘪𝘵 100/100 𝘳𝘦𝘢𝘤𝘩𝘦𝘥.", use_gif=True)
        ...
        if not parsed: return await styled_reply(event, f"⚠️ 𝘈𝘭𝘭 𝘱𝘳𝘰𝘹𝘪𝘦𝘴 𝘢𝘳𝘦 𝘢𝘭𝘳𝘦𝘢𝘥𝘺 𝘢𝘥𝘥𝘦𝘥 𝘰𝘳 𝘪𝘯𝘷𝘢𝘭𝘪𝘥.", use_gif=True)
        parsed = parsed[:100-cc]
        tm = await styled_reply(event, f"⦗ {CE_11} ⦘ 𝘈𝘥𝘥𝘪𝘯𝘨 𝘱𝘳𝘰𝘹𝘪𝘦𝘴...")
        added = 0
        for pd2 in parsed:
...
async def view_proxies(event):
    ...
    try:
        ...
        if not proxies: return await styled_reply(event, f"⚠️ 𝘠𝘰𝘶 𝘥𝘰𝘯'𝘵 𝘩𝘢𝘷𝘦 𝘢𝘯𝘺 𝘱𝘳𝘰𝘹𝘪𝘦𝘴 𝘴𝘢𝘷𝘦𝘥.", use_gif=True)
...
async def remove_proxy_cmd(event):
    ...
    try:
        if not await force_join_check(event): return
        proxies = await get_all_user_proxies(event.sender_id)
        if not proxies: return await styled_reply(event, f"⚠️ 𝘕𝘰 𝘱𝘳𝘰𝘹𝘪𝘦𝘴 𝘵𝘰 𝘳𝘦𝘮𝘰𝘷𝘦.", use_gif=True)
        p = event.raw_text.split(maxsplit=1)
        if len(p) == 1: return await styled_reply(event, f"⚠️ 𝘚𝘱𝘦𝘤𝘪𝘧𝘺 'all' 𝘰𝘳 𝘵𝘩𝘦 𝘱𝘳𝘰𝘹𝘺 𝘯𝘶𝘮𝘣𝘦𝘳.", use_gif=True)
        arg = p[1].strip().lower()
        if arg == 'all':
            c = await clear_all_proxies(event.sender_id)
            return await styled_reply(event, f"⦗ {CE_4} ⦘ 𝘊𝘭𝘦𝘢𝘳𝘦𝘥 <code>{c}</code> 𝘗𝘳𝘰𝘹𝘪𝘦𝘴 𝘴𝘶𝘤𝘤𝘦𝘴𝘴𝘧𝘶𝘭𝘭𝘺.", use_gif=True)
        try:
            idx = int(arg) - 1
            if 0 <= idx < len(proxies):
                rm = await remove_proxy_by_index(event.sender_id, idx)
                await styled_reply(event, f"⦗ {CE_4} ⦘ 𝘗𝘳𝘰𝘹𝘺 𝘳𝘦𝘮𝘰𝘷𝘦𝘥.", use_gif=True)
            else: await styled_reply(event, f"⚠️ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘱𝘳𝘰𝘹𝘺 𝘯𝘶𝘮𝘣𝘦𝘳.", use_gif=True)
        except Exception: await styled_reply(event, f"⚠️ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘱𝘳𝘰𝘹𝘺 𝘯𝘶𝘮𝘣𝘦𝘳.", use_gif=True)
    except Exception as e: await event.reply(f"⚠️ 𝘌𝘳𝘳𝘰𝘳: {e}")
...
async def generate_keys_cmd(event):
    ...
    if plan_key not in PLANS: 
        return await styled_reply(event, f"⚠️ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘗𝘭𝘢𝘯. 𝘜𝘴𝘦: plan1, plan2, plan3, plan4", use_gif=True)
...
async def redeem_key_cmd(event):
    ...
    if not code: return await styled_reply(event, f"⚠️ 𝘗𝘭𝘦𝘢𝘴𝘦 𝘱𝘳𝘰𝘷𝘪𝘥𝘦 𝘺𝘰𝘶𝘳 𝘬𝘦𝘺: <code>/redeem [𝘒𝘦𝘺]</code>", use_gif=True)
    
    keys_db = await load_keys()
    uid = event.sender_id
    
    if code not in keys_db:
        return await styled_reply(event, f"⚠️ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘒𝘦𝘺. 𝘗𝘭𝘦𝘢𝘴𝘦 𝘤𝘩𝘦𝘤𝘬 𝘢𝘯𝘥 𝘵𝘳𝘺 𝘢𝘨𝘢𝘪𝘯.", use_gif=True)
    
    kinfo = keys_db[code]
    if kinfo["used"]:
        return await styled_reply(event, f"⚠️ 𝘛𝘩𝘪𝘴 𝘒𝘦𝘺 𝘩𝘢𝘴 𝘢𝘭𝘳𝘦𝘢𝘥𝘺 𝘣𝘦𝘦𝘯 𝘳𝘦𝘥𝘦𝘦𝘮𝘦𝘥.", use_gif=True)
...
╰ ⦗ {CE_4} ⦘ 𝘌𝘹𝘱𝘪𝘳𝘦𝘴 𝘖𝘯: <code>{expiry_date}</code>"""
    ...
    await styled_reply(event, msg, use_gif=True, specific_gif=REDEEM_GIF)

    try:
        admin_notification = f"""⦗ 🔔 ⦘ 𝘕𝘦𝘸 𝘒𝘦𝘺 𝘙𝘦𝘥𝘦𝘦𝘮𝘦𝘥!
...
async def validate_key_cmd(event):
    ...
    if code not in keys_db: return await styled_reply(event, f"⚠️ 𝘒𝘦𝘺 𝘯𝘰𝘵 𝘧𝘰𝘶𝘯𝘥 𝘪𝘯 𝘥𝘢𝘵𝘢𝘣𝘢𝘴𝘦.", use_gif=True)
...
async def maint_cmd(event):
    ...
    if _MAINTENANCE_MODE: await styled_reply(event, f"⦗ {CE_11} ⦘ 𝘔𝘢𝘪𝘯𝘵𝘦𝘯𝘢𝘯𝘤𝘦 𝘔𝘰𝘥𝘦: 𝘖𝘕\n╰ 𝘈𝘭𝘭 𝘶𝘴𝘦𝘳𝘴 𝘢𝘳𝘦 𝘯𝘰𝘸 𝘣𝘭𝘰𝘤𝘬𝘦𝘥.", use_gif=True)
    else: await styled_reply(event, f"⦗ {CE_4} ⦘ 𝘔𝘢𝘪𝘯𝘵𝘦𝘯𝘢𝘯𝘤𝘦 𝘔𝘰𝘥𝘦: 𝘖𝘍𝘍\n╰ 𝘚𝘺𝘴𝘵𝘦𝘮 𝘪𝘴 𝘰𝘯𝘭𝘪𝘯𝘦 𝘧𝘰𝘳 𝘢𝘭𝘭 𝘶𝘴𝘦𝘳𝘴.", use_gif=True)
...
async def admin_users_cmd(event):
    ...
    active_info = []
    for u, p in ACTIVE_MTXT_PROCESSES.items():
        if not p.get("stopped"):
            try:
                user = await client_instance.get_entity(int(u))
                name = getattr(user, 'first_name', f"User {u}")
            except Exception:
                name = f"User {u}"
            
            gate = p.get("gate", "Unknown")
            total = p.get("total", "?")
            active_info.append(f"  ├ ⦗ 👤 ⦘ <a href='tg://user?id={u}'>{name}</a> (<code>{u}</code>)\n  │  ╰ 𝘎𝘢𝘵𝘦: <code>{gate}</code> | 𝘊𝘊𝘴: <code>{total}</code>")
            
    active_count = len(active_info)
    total_seen = len(USER_LAST_REQ)
    
    text = f"⦗ 🌐 ⦘ 𝘎𝘭𝘰𝘣𝘢𝘭 𝘚𝘺𝘴𝘵𝘦𝘮 𝘚𝘵𝘢𝘵𝘶𝘴\n\n├ ⦗ 👥 ⦘ 𝘛𝘰𝘵𝘢𝘭 𝘚𝘦𝘴𝘴𝘪𝘰𝘯 𝘜𝘴𝘦𝘳𝘴: <code>{total_seen}</code>\n├ ⦗ {CE_2} ⦘ 𝘈𝘤𝘵𝘪𝘷𝘦 𝘊𝘩𝘦𝘤𝘬𝘦𝘳𝘴: <code>{active_count}</code>\n"
    
    if active_info:
        text += "╰ ⦗ 🆔 ⦘ 𝘊𝘶𝘳𝘳𝘦𝘯𝘵𝘭𝘺 𝘊𝘩𝘦𝘤𝘬𝘪𝘯𝘨:\n" + "\n".join(active_info)
    else:
        text += "╰ ⦗ 🆔 ⦘ 𝘊𝘶𝘳𝘳𝘦𝘯𝘵𝘭𝘺 𝘊𝘩𝘦𝘤𝘬𝘪𝘯𝘨: <code>None</code>"
...
async def revoke_plan_cmd(event):
    ...
    except Exception: return await styled_reply(event, f"⚠️ 𝘗𝘭𝘦𝘢𝘴𝘦 𝘱𝘳𝘰𝘷𝘪𝘥𝘦 𝘢 𝘷𝘢𝘭𝘪𝘥 𝘐𝘋.", use_gif=True)
...
async def auto_file_check_cmd(event):
    ...
    if not (mime.startswith('text/') or mime in ['application/octet-stream', 'text/plain']):
        return
        
    processing_msg = await styled_reply(event, f"⦗ {CE_14} ⦘ 𝘗𝘳𝘰𝘤𝘦𝘴𝘴𝘪𝘯𝘨 𝘧𝘪𝘭𝘦 𝘥𝘢𝘵𝘢...", use_gif=True)
    
    try:
        if uid in ACTIVE_MTXT_PROCESSES and not ACTIVE_MTXT_PROCESSES[uid].get("stopped", True): 
            return await styled_edit(processing_msg, f"⚠️ 𝘈 𝘱𝘳𝘰𝘤𝘦𝘴𝘴 𝘪𝘴 𝘢𝘭𝘳𝘦𝘢𝘥𝘺 𝘢𝘤𝘵𝘪𝘷𝘦! 𝘗𝘭𝘦𝘢𝘴𝘦 𝘸𝘢𝘪𝘵.")
            
        if getattr(event.document, 'size', 0) > 3 * 1024 * 1024:
            return await styled_edit(processing_msg, f"⚠️ 𝘍𝘪𝘭𝘦 𝘵𝘰𝘰 𝘭𝘢𝘳𝘨𝘦! (𝘔𝘢𝘹 3𝘔𝘉)")
        if not await force_join_check(event): return await processing_msg.delete()
        
        plan = await get_user_plan(uid)
        db_proxies = await get_all_user_proxies(uid)
        if len(db_proxies) == 0:
            return await styled_edit(processing_msg, f"<b>⚠️ 𝘠𝘰𝘶 𝘮𝘶𝘴𝘵 𝘢𝘥𝘥 𝘱𝘳𝘰𝘹𝘪𝘦𝘴 𝘣𝘦𝘧𝘰𝘳𝘦 𝘤𝘩𝘦𝘤𝘬𝘪𝘯𝘨! 𝘜𝘴𝘦 <code>/addpxy</code></b>")
        ...
        if not cards:
            return await styled_edit(processing_msg, f"⚠️ 𝘕𝘰 𝘷𝘢𝘭𝘪𝘥 𝘤𝘢𝘳𝘥𝘴 𝘧𝘰𝘶𝘯𝘥 𝘪𝘯 𝘵𝘩𝘦 𝘧𝘪𝘭𝘦.")
        ...
        await styled_edit(processing_msg, f"⦗ {CE_4} ⦘ 𝘍𝘪𝘭𝘦 𝘓𝘰𝘢𝘥𝘦𝘥 𝘚𝘶𝘤𝘤𝘦𝘴𝘴𝘧𝘶𝘭𝘭𝘺\n\n├ 𝘛𝘰𝘵𝘢𝘭 𝘊𝘊𝘴: <code>{len(cards)}</code>\n╰ 𝘗𝘭𝘦𝘢𝘴𝘦 𝘴𝘦𝘭𝘦𝘤𝘵 𝘢 𝘎𝘢𝘵𝘦𝘸𝘢𝘺 𝘵𝘰 𝘴𝘵𝘢𝘳𝘵:", buttons=kb)
    except Exception as e:
        await styled_edit(processing_msg, f"⚠️ 𝘌𝘳𝘳𝘰𝘳: {e}")
...
async def gateway_selection_cb(event):
    ...
    if gate_name.startswith("soon_"): return await event.answer(f"⏳ 𝘎𝘢𝘵𝘦𝘸𝘢𝘺 𝘪𝘴 𝘤𝘰𝘮𝘪𝘯𝘨 𝘴𝘰𝘰𝘯!", alert=True)
    
    original_msg = await event.get_message()
    if gate_name == "cancel":
        PENDING_FILES.pop(uid, None)
        return await styled_edit(original_msg, f"⦗ {CE_7} ⦘ 𝘗𝘳𝘰𝘤𝘦𝘴𝘴 𝘊𝘢𝘯𝘤𝘦𝘭𝘭𝘦𝘥.", buttons=None)
    
    cards = PENDING_FILES.pop(uid, None)
    if not cards: return await event.answer("⚠️ 𝘚𝘦𝘴𝘴𝘪𝘰𝘯 𝘦𝘹𝘱𝘪𝘳𝘦𝘥 𝘰𝘳 𝘪𝘯𝘷𝘢𝘭𝘪𝘥 𝘧𝘪𝘭𝘦.", alert=True)
    
    ACTIVE_MTXT_PROCESSES[uid] = {"stopped": False, "tasks": [], "total": len(cards), "gate": gate_name}
    
    msg = await styled_edit(original_msg, f"⦗ {CE_11} ⦘ 𝘗𝘳𝘦𝘱𝘢𝘳𝘪𝘯𝘨 𝘚𝘦𝘴𝘴𝘪𝘰𝘯...\n\n├ 𝘓𝘰𝘢𝘥𝘦𝘥: <code>{len(cards)} 𝘊𝘊𝘴</code>\n├ 𝘛𝘩𝘳𝘦𝘢𝘥𝘴: <code>{WORKERS}</code>\n╰ 𝘎𝘢𝘵𝘦𝘸𝘢𝘺: <code>{gate_name}</code>", buttons=None)
...
async def _run_mass_process(event, msg_obj, cards, process_store, stop_prefix, gate_name):
    ...
    async def dashboard_updater():
        while not is_stopped():
            ...
            h_now = elapsed_now // 3600
            m_now = (elapsed_now % 3600) // 60
            s_now = elapsed_now % 60
            
            dashboard_text = f"""⦗ {CE_2} ⦘ 𝘚𝘦𝘴𝘴𝘪𝘰𝘯 𝘈𝘤𝘵𝘪𝘷𝘦...
        ...
        while not queue.empty() and not is_stopped():
            ...
            try:
                ...
                if status == 'Charged':
                    ...
                    asyncio.create_task(_send_global_hit("Charged", gate_name, res.get('message', ''), res.get('price', '-'), uid, card_el))
                    ...
                    asyncio.create_task(_send_global_hit("Insufficient", gate_name, res.get('message', ''), res.get('price', '-'), uid, card_el))
...
async def stop_chk_cb(event):
    ...
    if event.sender_id != puid and event.sender_id not in ADMIN_ID: return await event.answer("⚠️ 𝘕𝘰𝘵 𝘺𝘰𝘶𝘳𝘴!", alert=True)
...
async def main():
    ...
    while True:
        try:
            ...
            print("✅ VIP BOT IS FULLY OPERATIONAL WITH ZERO ERRORS!")
