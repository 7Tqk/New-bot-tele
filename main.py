# 𝘚𝘩𝘰𝘱𝘪𝘧𝘺 𝘝𝘐𝘗 𝘚𝘺𝘴𝘵𝘦𝘮 - (𝟰𝟬𝗪 - 𝟭𝟬𝟬% 𝘈𝘤𝘤𝘶𝘳𝘢𝘤𝘺 - 𝘡𝘦𝘳𝘰-𝘓𝘢𝘵𝘦𝘯𝘤𝘺 - 𝘗𝘳𝘪𝘮𝘢𝘳𝘺 𝘉𝘭𝘶𝘦 𝘜𝘐)
...
VIP_EMOJIS = {
    ...
    "total": "5042212959149474775", "speed": "5042187654402122602",
...
CUSTOM_COUNTRY_EMOJIS = {"US": 0, "GB": 0, "SA": 0, "AE": 0}
...
async def _send_global_hit(status, gateway, message, price, uid):
    try:
        ...
        if status == "Charged": header = f"⦗ {get_custom_emoji('charged', '🟢')} ⦘ 𝘊𝘏𝘈𝘙𝘎𝘌𝘋 𝘚𝘜𝘊𝘊𝘌𝘚𝘚𝘍𝘜𝘓𝘓𝘠"
        elif status == "Insufficient": header = f"⦗ {get_custom_emoji('insufficient', '🟡')} ⦘ 𝘐𝘕𝘚𝘜𝘍𝘍𝘐𝘊𝘐𝘌𝘕𝘛 𝘍𝘜𝘕𝘋𝘚"
        else: return 
...
async def add_proxy_cmd(event):
    ...
    try:
        ...
        if event.is_reply:
            ...
            else: return await styled_reply(event, f"⦗ 💎 ⦘ 𝘗𝘭𝘦𝘢𝘴𝘦 𝘱𝘳𝘰𝘷𝘪𝘥𝘦 𝘵𝘩𝘦 𝘱𝘳𝘰𝘹𝘪𝘦𝘴 𝘤𝘰𝘳𝘳𝘦𝘤𝘵𝘭𝘺.", use_gif=True)
        ...
        if cc >= 100: return await styled_reply(event, f"⦗ 💎 ⦘ 𝘓𝘪𝘮𝘪𝘵 100/100 𝘳𝘦𝘢𝘤𝘩𝘦𝘥.", use_gif=True)
...
async def remove_proxy_cmd(event):
    ...
    try:
        ...
        try:
            ...
            else: await styled_reply(event, f"⦗ 💎 ⦘ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘱𝘳𝘰𝘹𝘺 𝘯𝘶𝘮𝘣𝘦𝘳.", use_gif=True)
        except Exception: await styled_reply(event, f"⦗ 💎 ⦘ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘱𝘳𝘰𝘹𝘺 𝘯𝘶𝘮𝘣𝘦𝘳.", use_gif=True)
    except Exception as e: await event.reply(f"⚠️ Error: {e}")
...
async def generate_keys_cmd(event):
    ...
    if plan_key not in PLANS: return await styled_reply(event, f"⦗ 💎 ⦘ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘗𝘭𝘢𝘯. 𝘗𝘭𝘦𝘢𝘴𝘦 𝘶𝘴𝘦: plan1, plan2, plan3, plan4", use_gif=True)
    pi = PLANS[plan_key]
...
async def redeem_key_cmd(event):
    ...
    keys_db = await load_keys()
    uid = event.sender_id
    if code not in keys_db: return await styled_reply(event, f"⦗ 💎 ⦘ 𝘐𝘯𝘷𝘢𝘭𝘪𝘥 𝘒𝘦𝘺. 𝘗𝘭𝘦𝘢𝘴𝘦 𝘤𝘩𝘦𝘤𝘬 𝘢𝘯𝘥 𝘵𝘳𝘺 𝘢𝘨𝘢𝘪𝘯.", use_gif=True)
    kinfo = keys_db[code]
    if kinfo["used"]: return await styled_reply(event, f"⦗ 💎 ⦘ 𝘛𝘩𝘪𝘴 𝘒𝘦𝘺 𝘩𝘢𝘴 𝘢𝘭𝘳𝘦𝘢𝘥𝘺 𝘣𝘦𝘦𝘯 𝘳𝘦𝘥𝘦𝘦𝘮𝘦𝘥.", use_gif=True)
    tier = kinfo["tier"]
    days = kinfo["days"]
...
async def auto_file_check_cmd(event):
    ...
    try:
        ...
        kb = [
            [Button.inline("🟦 𝘚𝘩𝘰𝘱𝘪𝘧𝘺 (𝘊𝘩𝘢𝘳𝘨𝘦)", b"gate:Shopify"), Button.inline("🌐 𝘉𝘳𝘢𝘪𝘯𝘵𝘳𝘦𝘦 (𝘚𝘰𝘰𝘯)", b"gate:soon_Braintree")],
    ...
    async def worker():
        ...
        while not queue.empty() and not is_stopped():
            ...
            try:
                ...
                queue.task_done()
                await asyncio.sleep(1.0) # تأخير إجباري لمنع الانفجار وتحقيق انسيابية كاملة
